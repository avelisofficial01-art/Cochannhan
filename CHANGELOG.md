...
| Không thấy quái ở Làng Cổ Thảo | Tọa độ thỏ tuyết trong `mapSpawnSeeds` ở `lang_cothao` nằm ngoài biên bản đồ (x=1600, y=1600 trong khi map chỉ 1500x1500px). Đồng thời DB seeding bỏ qua mapSpawns nếu worldMaps đã có dữ liệu. | Dời tọa độ thỏ tuyết vào trong biên bản đồ (x=1200, y=1200) và cập nhật `seed.ts` để luôn refresh lại bảng `mapSpawns` mỗi lần khởi động server. |
| Không biết hướng đi map khác | Các cổng dịch chuyển nằm sát rìa bản đồ, camera bị giới hạn theo tầm nhìn người chơi nên các cổng này bị ẩn ngoài màn hình mà không có chỉ hướng. | Thêm hệ thống chỉ hướng cổng dịch chuyển ngoài màn hình (`drawPortalPointers`) vẽ các mũi tên cyan xoay động hướng về phía cổng dịch chuyển ở viền camera. |
...
| `backend/src/database/seed.ts` | Cập nhật logic seed để luôn clear và nạp lại mapSpawns. |
