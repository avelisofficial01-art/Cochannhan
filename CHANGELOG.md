...
| S27.6 | Seed/schema hardening | ✅ |

### Hardening bổ sung
- Viết lại `backend/src/database/seedData.ts` và `backend/src/database/seed.ts` theo schema hiện tại.
- Sửa compatibility layer cho repository inventory/monster/npc/quest/world/shop để giữ API contract trong khi DB schema đã thay đổi.
- Verification: `npm run build`, `npm run lint`, `npm run typecheck` pass.
